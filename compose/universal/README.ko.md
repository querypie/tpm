...
재시작 방법은 변경 내용에 따라 다릅니다.

### 컨테이너 설정 변경 시 (`.env`, `compose.yml` 수정 등)
...
### 내부 설정만 변경 시 (DB 설정 변경 등)

컨테이너 설정은 그대로 두고 프로세스만 재시작합니다.
...
`configure-proxy.sh` 스크립트를 사용하면 DAC/SAC 및 KAC Proxy 주소를 자동으로 설정할 수 있습니다.
...
자세한 내용은 [설치 후 초기 설정](https://docs.querypie.com/ko/installation/post-installation-setup) 문서를 참조하세요.
